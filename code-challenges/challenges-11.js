...
function square(arr) {
    let squareArray =arr.map(function(v,i){
        return v*v;
    });
...
function fullName(arr) {
    let studentsnames = arr.map((fn , i) => `${fn.firstName} ${fn.lastName}`);
...
function gradesAvg(arr) {
    ...
    arr.map(function (v, i) {
     let gradesum = 0;
     let gradeavg = 0;
     for (let x of v.gradsList) {
       gradesum += x;
     }
     gradeavg = gradesum / v.gradsList.length;
     v.avg = gradeavg;
   })
   return arr;
